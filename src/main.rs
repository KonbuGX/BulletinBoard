...
use models::EditAccountNameParams;
use models::EditPasswordParams;
...
use service::get_acct_no;
use service::insert_account;
use service::update_account_info;
use service::update_password;
use service::remove_account;
use service::validation_account;
use service::validation_account_name;
use service::validation_password;
...
//ログイン中のアカウントのページ表示
#[get("/mypage")]
async fn mypage(redis: web::Data<r2d2_redis::r2d2::Pool<r2d2_redis::RedisConnectionManager>>) -> Result<HttpResponse,MyError>{
    //ログインの状態を取得
    let mut redis_conn = redis.get()?;
    let acct_info = get_session(&mut redis_conn, &SESSION_ID);
    let login_status = get_login_status(&acct_info);
...
//ログイン中のアカウントのページ表示
#[get("/account")]
async fn account(redis: web::Data<r2d2_redis::r2d2::Pool<r2d2_redis::RedisConnectionManager>>) -> Result<HttpResponse,MyError>{
    //ログインの状態を取得
    let mut redis_conn = redis.get()?;
    let acct_info = get_session(&mut redis_conn, &SESSION_ID);
    let login_status = get_login_status(&acct_info);
...
//アカウントページで変更ボタン押下時
#[post("/account")]
async fn edit_account(params: web::Form<EditAccountNameParams>,redis: web::Data<r2d2_redis::r2d2::Pool<r2d2_redis::RedisConnectionManager>>,db: web::Data<r2d2::Pool<ConnectionManager<SqliteConnection>>>) -> Result<HttpResponse,MyError>{
    let conn = db.get()?;
    let mut redis_conn = redis.get()?;
...
//ログイン中のアカウントのページ表示
#[get("/password")]
async fn password(redis: web::Data<r2d2_redis::r2d2::Pool<r2d2_redis::RedisConnectionManager>>) -> Result<HttpResponse,MyError>{
    //ログインの状態を取得
    let mut redis_conn = redis.get()?;
    let acct_info = get_session(&mut redis_conn, &SESSION_ID);
    let login_status = get_login_status(&acct_info);
...
//アカウントページで変更ボタン押下時
#[post("/password")]
async fn edit_password(params: web::Form<EditPasswordParams>,redis: web::Data<r2d2_redis::r2d2::Pool<r2d2_redis::RedisConnectionManager>>,db: web::Data<r2d2::Pool<ConnectionManager<SqliteConnection>>>) -> Result<HttpResponse,MyError>{
    let conn = db.get()?;
    let mut redis_conn = redis.get()?;
...
async fn main() -> Result<(),actix_web::Error> {
    ...
    .service(mypage).service(account).service(edit_account).service(password).service(edit_password)
